...
dbdt = df.loc[:,'DBDT_1':'DBDT_32'].values
lbl = df.loc[:,'DBDT_INUSE_10':'DBDT_INUSE_10'].values
#check if all lbls are equal
lbl2 = np.transpose(lbl)
print("All labels are identical: " + str((lbl2 == lbl2[0,:]).all()))

#find idex of label change from 0 to 1 or 1 to 0. Ved at finde indeks for en inbyrdes non-zero differens
# dbdt = [1, 1, 1, 2, 2, 1, 2, 2]
# b = [0, 0, 0, 1, 1, 0, 1, 2]
...
#         startIdx = idx + 1
#         endIdx = zero_crossings[i + 1] + 1
#         plt.plot(x[startIdx : endIdx], dbdt[startIdx : endIdx, :], color = cflag)
#     #plot very last segment
#     else:
#         plt.plot(x[idx + 1 : ], dbdt[idx + 1 : , :], color = cflag)
# plt.xlim(200,700)
# plt.show()
# exit()